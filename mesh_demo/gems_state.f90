...
contains
   ...
   function rhoif(qv)
      ...
      case(0) ! perfect gas
         ...
         do i = 1, nspe
            ...
            case(1)
               rhoif(i) = 1.01e5 * g_mwi(i) / (g_runi * 300.)
               ! rhoif(i) = g_rho0(i)
   ...
   !derivative of density w.r.t pressure, temperature and concentration
   function rhoxf(qv)
   ...
   function hif(qv)
      type(vector), intent(in) :: qv
      real(rfp) :: hif(nspe), t, t_inv, tln, cp, a1, a2, tr, p
      integer :: i, j, n
      type(Qvar) :: pro
      ...
      case(10) ! imcompressible
         hif = g_hrefi + g_cpi * (qv % v(ien) - g_htref)
   ...
   !derivative of enthalpy w.r.t pressure, temperature and concentration
   function hxf(qv)
      type(vector) :: qv, hxf
   ...
   !try to modify qv such that the desired rho and h0 can be obtained from qv
   subroutine qvfromh0(rho, h0, qv, qvl, qvr)
      ...
      ! correct temperature by ethalpy
      dh = h - hf(qv)
      d1 = dh / htf(qv)
      qv % v(ien) = d1 + qv % v(ien)

      ! correct pressure by density
      if(rhopf(qv) /= 0) then        
         drho = rho - rhof(qv)
         d2 = drho / rhopf(qv)
   ...
   function liquid_fraction(qv)result(g)
      real(rfp) :: g
      type(vector) :: qv
      ! linear liquid fraction model for stainless steel
      ! calculate mass fraction of liquid
   ...
   function liquid_fraction_t(qv)result(g)
      real(rfp) :: g, t
      type(vector) :: qv
      ! the derivative of liquid fraction with respect to temperature
      t = qv % v(ien)
