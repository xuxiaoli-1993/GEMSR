...
contains
	...
	!set up fluid and other (metal, laser, level-set, etc.) properties
        call fluid_property
	...
	!by Wenda?
	!if it is Cartesian coordinates, find xc(:), yc(:), and zc(:)
	...
	!see module dfd_constant for constants
        print *, '-------', id, neq
    ...
    subroutine fluid_property
        ...
        if (usels > 0) then ! read in metal property
	  select case(ls_material)
	    case(110)
            call metal_property
            call laser_property
	...
	if (usels > 0) then ! level-set initialization
	  call levelset_allocation(cells,faces,interf)
        end if
	    ...
	    if(usels>0) then
	      call levelset_initialization(cells,faces)
	    end if
        end if
	    ...
	    !cool the substrate (by Wenda?)  
            if (s_nstart > 0) then
                ...
                !read level-set from binary
                if(usels>0) then
		  do i=1,size(cells)
		    read(5, end = 10) cells(i)%ls
		  end do
		end if
            end if
        end if
    ...
    subroutine output_option
        ...
        !Note: level-set can only be plotted by output_cell_tecplot
        if (usels == 0) ibuff(55) = 0
        do i = 1, size(ibuff)
            if (ibuff(i) == 1) then
    ...
    subroutine setup_extension(cells, faces, id, np)
        ...
        integer :: xvalue(500) = 1e8, yvalue(500) = 1e8, zvalue(500) = 1e8
        integer :: xvaluegather(10000) = 1e8, yvaluegather(10000) = 1e8, zvaluegather(10000) = 1e8
        ...
        do i = 1, xctmax * np
            isize_ext = i
            if (xvaluegather(i + 1) >= 1e8 - 10) exit
        end do
        ...
        do i = 1, yctmax * np
            jsize_ext = i
            if (yvaluegather(i + 1) >= 1e8 - 10) exit
        end do
        ...
        do i = 1, zctmax * np
            ksize_ext = i
            if (zvaluegather(i + 1) >= 1e8 - 10) exit
        end do
    ...
    subroutine rearragement(input, length)
        ...
        do i = 2, length
            do j = 1, i - 1
                if (input(i) == input(j) .and. input(i) <= 1e8 - 10) then
