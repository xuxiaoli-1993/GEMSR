...
!
  ...
  do j = 1, size(output_selection)
   ...
   select case(j)
   case(1)
    write(2,20)cells(:)%dqv%v(ico)
   case(2)
    write(2,20)(pf(cells(i)%qv),i=1,n)
   case(3)
    write(2,20)(stagnation_pressure(cells(i)%qv,cells(i)%centp),i=1,n)
   case(5)
    write(2,20)cells(:)%dqv%v(imb)
   case(6)
    write(2,20)cells(:)%dqv%v(imb+1)
   case(7)
    if(naux > 0) then
    write(2,20)cells(:)%dqv%v(iaub)
    else
    write(2,20)cells(:)%dqv%v(imb+2)
    end if
   case(8)
    write(2,20)cells(:)%dqv%v(ien)
   case(9)
    write(2,20)(stagnation_temperature(cells(i)%qv,cells(i)%centp),i=1,n)
   case(10)
    write(2,20)cells(:)%dqv%v(ike)
   case(11)
!    write(2,20)cells(:)%dqv%v(iom)
   ...
   case(25)
    write(2,20)cells(:)%dqv%v(irad)
!   case(26)
!    write(2,20)nodevalue(cells,nodes,faces,interf,1)  ! shadowgraph
   case(27)
    write(2,20)cells(:)%dqv%v(ibb)
   case(28)
    write(2,20)cells(:)%dqv%v(ibb+1)
   case(29)
    write(2,20)cells(:)%dqv%v(ibe)
   case(30)
    write(2,20)cells(:)%dqv%v(ieb)
!    write(2,20)nodevalue(cells,nodes,faces,interf,20+ieb) 
   case(31)
    write(2,20)cells(:)%dqv%v(ieb+1)
   case(32)
    write(2,20)cells(:)%dqv%v(iee)
   case(33)
    write(2,20)cells(:)%dqv%v(ifb)
   case(34)
    write(2,20)cells(:)%dqv%v(ife)
!   case(35)
!    write(2,20)nodevalue(cells,nodes,faces,interf,5)  
!   case(36)
!    write(2,20)nodevalue(cells,nodes,faces,interf,6) 
...
subroutine wenda_output(cells,faces,np,id,nadv,freq) 
    implicit none
    type(face),pointer::faces(:),cf
    type(cell),pointer::cells(:),cc
    integer::np,id,nadv,freq,index   
        ...
        do j=1,jsize_ext
            do k=1,ksize_ext
                if (map_ext(i,j,k)>0) then
                    var1_temp(i,j,k) = cells(map_ext(i,j,k))%qv%v(ils) 
                    ...
                    var6_temp(i,j,k) = cells(map_ext(i,j,k))%curvature
                    var7_temp(i,j,k) = cells(map_ext(i,j,k))%vinterface2
                end if            
            end do
        end do     
    end do
...
!6   format (1x,20e20.6)
    ...
    if (id==0) then           
        print *,'printing wenda_output with K'
        index=nadv/freq
        fn = "wenda_output_"//trim(i2s(index))//".dat"     
        open(43,file=fn)
        write(43,*) 'TITLE="vector"'
        write(43,*) 'VARIABLES="x(m)" "y(m)" "z(m)" "levelset" "T" "Vx" "Vy" "Vz" "VAR1" "VAR2"'
        write(43,*) 'ZONE T="',int(nadv/s_nt), '" I=',ISIZE_ext,' J=',JSIZE_ext,' K=',KSIZE_ext,' F=POINT'
        DO k=1,KSIZE_ext
        DO j=1,JSIZE_ext
        DO i=1,ISIZE_ext    
