...
contains
    ...
    subroutine boundary_condition(faces, nodes, cells, interf)
        ...
        do i = ib, size(faces)
            ...
            case(wall_type)
                call wall_boundary(cf, mybc, nodes)
            case(geom_topology)
            ...
            ! level set
            if (s_ilevelset > 0) cf % right_cell % qv % v(ils) = cf % left_cell % qv % v(ils)
        end do
        !
    ...
    subroutine inlet_boundary(cf, bc)
        ...
        case(0, 1, 8)
            ...
            qvr % v(ien + 1:neqf) = bc % vars(ien + 1:neqf)
            !
        case(2)
            qvr % v(:neqf) = bc % vars(:neqf)
    ...
    subroutine inlet_user_define(cf, bc)
        ...
        qvr % v(:neqf) = bc % vars(:neqf)
        qvr % v(ico) = qvl % v(ico)
        if (cf % centp(2) / b_lenref < .5) then
            qvr % v(ien) = 13000. - 1.e4_rfp * cf % centp(2) / (0.5 * b_lenref)
            !    qvr%v(imb) = 60._rfp
        else
            qvr % v(ien) = max(1000., 3000. - 2.e3_rfp * (cf % centp(2) / b_lenref - 0.5) / 2.0)
            !    qvr%v(ien) = 3000._rfp
        end if
        qvr % v(imb:ime) = -bc % vars(neq + 4) / rhof(qvr) * cf % vecn
        !   qvr%v(ien) = bc%vars(ien)*exp(-(cr%centp(2) - 2.0 * b_lenref)*200.) + 1000.
    ...
    subroutine wall_boundary(cf, bc, nodes)
        implicit none
        type(node), pointer :: nodes(:)
        type(bc_type), pointer :: bc
        type(face), pointer :: cf
        ...
        !
        if (g_igrav > 0) then
            qvr % v(ico) = qvl % v(ico) + rhof(qvl) * sum((cr % centp(i) - cl % centp(i)) * &
    ...
    subroutine inlet_jacob(cf, a)
        ...
        it = mod(bc(cf % itype) % itype, 1000)
        select case(it)
        case(0, 1, 5, 8) ! Specify H^0 and S
            rho = rhof(qv)
    ...
    subroutine outlet_jacob(cf, a)
        ...
        it = mod(bc(cf % itype) % itype, 1000)
        if (it == 1) return ! extrapolate boundary condition    
        ...
        do i = 1, interf % nc
            ...
            do j = 1, interf % ncp(i)
                ...
                do k = 1, size(cc % sface)
                    ...
                    if (bc(it) % igrp == geom_topology.and.mod(bc(it) % itype, 1000) == 2) exit
                end do
                sb(nd) = cc % vol
    ...
    subroutine update_interface_other(faces, nodes, cells, interf, dataname)
        implicit none
        type(itf) :: interf
        type(face), pointer :: faces(:)
        type(node), pointer :: nodes(:)
        type(cell), pointer :: cells(:), cc
        real(rfp), allocatable :: sb(:), rb(:)
        integer :: dataname, i, ist, n, ib, j, ip, nd, ierr, st(mpi_status_size), neq1, ir, meq, op

        !Wenda: interf%nitf is the number of interface faces
        !Wenda: interf%nn = tnn is the number of partitions sending information to this partition
        !Wenda: interf%np is the index of partitions sending information to this partition
        !Wenda: interf%nnp is the number of information sent from each of those partitions to this partition
        !Wenda: interf%nc is the total number of partitions receiving information from this partition
        !Wenda: interf%cp is the index of each of those partitions receiving informaiton from this partition
        !Wenda: tnc is the total number of partitions that receive information from this partition,
        !Wenda: interf%ncp = clist(tnc+1:2*tnc) + clist(2*tnc+1:3*tnc) is the number of cells and faces send to each partition    
        !
        call mpi_comm_rank(mpi_comm_world, ip, ierr)
        !
        ib = 0; ir = 0
        do i = 1, interf % nc
            op = interf % np(i)
            meq = 1 !Wenda: sending single value
            if (dataname == 2) meq = 3
            !     
            nd = meq * interf % ncp(i)
            neq1 = meq - 1
            ...
            do j = 1, interf % ncp(i)
                ...
                if (dataname == 5) sb(nd) = cc % curvature
            ...
            do j = 1, interf % nnp(i)
                ...
                if (dataname == 5) cc % curvature = rb(nd)

                nd = nd + meq
            end do
            deallocate(rb, sb)
        end do
    end subroutine update_interface_other


end MODULE dfd_bound
