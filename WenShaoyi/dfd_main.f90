...
PROGRAM dfd_main
    ...
    if (s_ilevelset > 0) call levelset_thickness(cells, nodes) ! level set
    ...
    do ischedule = 1, schedule % n
        do nadv = s_nstart, s_nend
            !
            if (id == 0) print *, 'ct and time', nadv, s_elapsed_time
            ...
            if (lsonly == 0) call calculate_curvature(cells, faces, nodes, id, nadv) !in disc.f90
            if (lsonly == 0) call update_interface_other(faces, nodes, cells, interf, 5) !update curvature 
            ...
            if (mod(nadv, s_nt) == 0) then
                call calculate_vinterface(cells, nadv, id)
                call Vint_extension(cells, faces, id, nadv)
                call levelset_calculation(cells, nadv) !in source.f90       
            end if
            ...
            !  if(mod(nadv,s_nplot) == 0) call output_to_plot(cells,nodes,faces,interf,id)
