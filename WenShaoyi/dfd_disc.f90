...
contains
    ...
    subroutine face_gradient(cf, rcell, lcell, nodes, gqv, vjacob)
        ...
        !  else
        !   call face_gradient_dli1(cf,rcell,lcell,nodes,gqv,vjacob)
        !  end if
    ...
    subroutine face_gradient_dli2(cf, rcell, lcell, nodes, gqv, vjacob)
        ...
        type(node) :: nodes(:)
        real(rfp), optional :: vjacob(ndim, 2)
        type(vector) :: gqv(:), dqv(ndim)
        integer :: k

        dqv(1) = rcell % qv - lcell % qv
        select case(size(nodes))
        case(2)
            dqv(2) = nodes(2) % qv - nodes(1) % qv
        case(3)
            dqv(2) = nodes(2) % qv - nodes(1) % qv
            dqv(3) = nodes(3) % qv - nodes(2) % qv
        case default
            dqv(2) = nodes(3) % qv - nodes(1) % qv
            dqv(3) = nodes(4) % qv - nodes(2) % qv
        end select
        !
        do k = 1, neq
...
subroutine cal_node_qv(cells, nodes, faces, interf, bnodes)
    ...
    do i = 1, size(cells)
        ...
        do j = 1, size(c2n)
            n = c2n(j)
            !   if(cells(i)%weight(j) < 1.e-30_rfp) cycle
...
subroutine barth_limiter(cells, faces, nodes)
    ...
    do i = 1, size(cells)
        ...
        do j = 1, size(current_cell % scell)
            ghost_cell => current_cell % scell(j) % to_cell
            qvn = ghost_cell % qv
            where(qvmax % v < qvn % v) qvmax % v = qvn % v
                where(qvmin % v > qvn % v) qvmin % v = qvn % v
                end do
                !
                phi = one
                ...
                do j = 1, size(current_cell % scell)
                    ...
                    where(qvmax % v < qvn % v) qvmax % v = qvn % v
                        ...
                        do j = 1, size(current_cell % scell)
                            ghost_cell => current_cell % scell(j) % to_cell
                            qvn % v = matmul(current_cell % gradient, ghost_cell % centp - cc)
                            where(qvn % v > qvmax % v) phi = two
                                where(qvn % v < qvmin % v) phi = two
                                end do
                        ...
                        subroutine calculate_curvature(cells, faces, nodes, id, nadv)
                            type(node), intent(in) :: nodes(:)
                            type(cell), pointer :: cells(:), cc, ghost_cell
                            type(face), pointer :: faces(:), cf, nbf
                            integer :: id, nadv

                            integer :: i, j, k, flag, flag2, a, b, c
                            real(rfp) :: vecn1(ndim), vecn2(ndim), Nxgrad(ndim), Nygrad(ndim), Nzgrad(ndim)

                            !    integer::i,j,k,ii,jj,kk,nbi,nbj,nbk,flag,flag1,flag2,flag3,ierr,doing,pt,ptall,output,time,freq
                            !    double precision::dist,dist2,total1,total2,total3,total4,total5,total6,ct,ct2,trash,dl,spe,cosine,maxcosine,sine,tangent
                            !    double precision::Psat,fp,precoil,F1,F2,newTsurf,newPsurf
                            !    double precision::ls1,ls2,t1,t2,vx1,vx2,vy1,vy2,vz1,vz2,totalt,totalconv,totalevap,totalV(3),totalVnormal,totalVgas,totalP,totalT2,totalP2
                            !    double precision::rrr,ttt,r,centerx,centerz,radius,ee1,ee2,Tmaxhere,vmax,vel(3),length,locx,locy,locz,deltals
                            ...
                            do i = 1, size(cells)
                                ...
                                Nygrad = zero
                                Nzgrad = zero
                                vecn1 = cells(i) % gradient(ils,:)
                                vecn1 = vecn1 / sqrt(sum(vecn1**2))
                                do j = 1, size(cells(i) % scell)
                                    ghost_cell => cc % scell(j) % to_cell
                                    vecn2 = ghost_cell % gradient(ils,:)
                                    if (sum(vecn2**2) <= 0) then
                                        vecn2 = vecn1
                                    end if
                                    vecn2 = vecn2 / sqrt(sum(vecn2**2))

                                    do k = 1, s_nrec !Wenda: s_nrec=3    
                                        Nxgrad(k) = Nxgrad(k) + (vecn2(1) - vecn1(1)) * cc % svd(j, k)
                                        Nygrad(k) = Nygrad(k) + (vecn2(2) - vecn1(2)) * cc % svd(j, k)
                                        Nzgrad(k) = Nzgrad(k) + (vecn2(3) - vecn1(3)) * cc % svd(j, k)
                                    end do

                                end do ! end loop neighbor      

                                cc % curvature = Nxgrad(1) + Nygrad(2) + Nzgrad(3)
