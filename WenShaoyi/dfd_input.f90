...
contains
    ...
    subroutine read_data(nodes, cells, faces, interf, pinterf, raw, bnodes)
        ...
        if (id == 0) then
            ...
            print *, 'number of faces=', size(faces)
            print *, 'number of nodes=', size(nodes)
        end if
        !
        call geometry(nodes, cells, faces) ! cal geometry information
        ...
        call initial(cells, faces, nodes, bnodes)
        call output_option
        ...
        !
        ! 
        !  if(s_ifmt /= 1) call face_geom(nodes,cells,faces)  ! cal geometry information
        call face_reconstruction(faces, nodes) ! cal geometry information
        !  call initial_cell_gradient(cells,faces)  ! cal geometry information

        call setup_extension(cells, faces, id, np)

    end subroutine read_data
    !
    subroutine levelset_thickness(cells, nodes)
        ...
        do i = 1, size(cells)
            ...
            !     fi0 = nodes(c2n(1))%qv%v(ils)   ! first fi 
            !    do j = 2, size(c2n)
            !     fi1 = nodes(c2n(j))%qv%v(ils)   ! first fi 
            !     if(fi0 * fi1 < 1.e-10_rfp) then
            !      vol = cc%vol
            !      if(ndim==2.and.s_iaxis > 0) vol = vol / cc%centp(s_iaxis)
            !      vol = vol**(one/real(ndim,rfp))
            !      dl = max(vol,dl)
            !      exit
            !     end if
            !   end do
        end do
        !    print *,'level set thickness =',dl
        !    dummy%k0 = dl * (one + half)   ! 1.5 time grid size
        dummy % k0 = dl * two * two ! two time grid size
        !    dummy%k0 = 2.5e-4 
        print *, 'level set thickness =', dummy % k0
    ...
    subroutine grid_information(nodes, cells, faces)
        ...
        write(32, geom)
        if (nd /= ndim) then
            print *, 'please change the ndim in dfd_constant.f90 to ', nd
    ...
    subroutine solution_information
        ...
        ! Solution control
        !
        integer :: irestart, nsteps, init, ialg, iscr, imeth, irec, iplot, nplot
        ...
        integer :: istatus, icolor, ifile, ipre, ischeme, ischedule, minput, ilevelset
        integer :: i, j
        !
        real(rfp) :: rbuff(20)
        integer :: ibuff(30)
        ...
        ipre, pbase, b0, nplot, ilevelset, &
        ivis, irea, iplot, isource, iperiodic, iaux, minput
        ...
        equivalence(ibuff(22), ilevelset)
        !
        ! for real
        ...
        equivalence(rbuff(15), cflm), (rbuff(16), bodyvel(1))
        integer :: id, ierr
        call mpi_comm_rank(mpi_comm_world, id, ierr)
        !  defaults
        !  if(id == 0) then
        ...
        s_iaux = iaux
        s_ilevelset = ilevelset ! level set
        !
        s_iscr = iscr
        s_ifile = ifile
        s_iplot = iplot
        ...
        if (s_ivis == 2.and.id == 0) then
            ...
            if (nteq /= 2) then
                print *, 'please set nteq = 2 in dfd_constant.f90'
                print *, 'Re-compile before you run the case'
                call mpi_abort(mpi_comm_world, ierr, ierr)
            end if
        end if
    ...
    subroutine fluid_property
        ...
        case(110)
            call metal_property
            call laser_property
            call powderflow_property
        end select

        if (s_ilevelset > 0) then ! set dummy property for level set solver

            dummy % rhol = 2000._rfp !2600._rfp
            dummy % mul = 1.e - 5 !1.e-5_rfp
            dummy % kl = 0.44 !1.e-2_rfp
            dummy % cpl = 780. !780._rfp   !1006.0_rfp
            dummy % beta = 1.e - 5_rfp ! stored rhop for dummy
            dummy % alpha = 1.e - 5_rfp !-one / 297._rfp  ! sotred rhot for dummy
            dummy % k0 = 1.e - 10_rfp
        end if
        !
    end subroutine fluid_property
    !
    subroutine metal_property
        implicit none
        ! fluid property
        real :: rhol, rhos, cpl, cps, lheat, kl, ks, mul, beta, sigma, sigmat, k0, dl, alpha, emi
        real :: tl, ts, gv(ndim)
        namelist /solid_prop/rhol, rhos, cpl, cps, lheat, kl, ks, mul, beta, sigma, sigmat, k0, dl, alpha, emi, &
        tl, ts, gv
        integer :: id, ierr
        if (s_irealgas /= 110) return
        call mpi_comm_rank(mpi_comm_world, id, ierr)
        ...
        metal % gravity = gv
        !
    end subroutine metal_property

    subroutine laser_property
        implicit none
        ! laser property
        real :: p, rb, A, x0, z0, vx, vz, x, y, z, zigzag, switch, thick, patch, length, dtpass, dtlayer, passes, cold, laseron
        namelist /laser_prop/p, rb, A, x0, z0, vx, vz, x, y, z, zigzag, switch, thick, patch, length, dtpass, dtlayer, passes, cold,  laseron
        integer :: id, ierr
        if (s_irealgas /= 110) return
        call mpi_comm_rank(mpi_comm_world, id, ierr)
        p = 500._rfp
        ...
        laser1 % y = y
        laser1 % z = z
        laser1 % zigzag = zigzag
        laser1 % switch = switch
        laser1 % layer_thickness = thick
        laser1 % patch_dist = patch
        laser1 % pass_length = length
        laser1 % time_between_passes = dtpass
        laser1 % time_between_layers = dtlayer
        laser1 % passes_per_layer = passes
        laser1 % cold_start = cold
        laser1 % laser_on = laseron
        !
    end subroutine laser_property

    subroutine powderflow_property
        implicit none
        ! laser property
        real :: c, vp, A, B, rp, h
        namelist /powderflow_prop/c, vp, A, B, rp, h
        integer :: id, ierr
        if (s_irealgas /= 110) return
        call mpi_comm_rank(mpi_comm_world, id, ierr)
        c = 20._rfp
        ...
        rp = 0.002_rfp
        h = 10.0_rfp
        read (11, powderflow_prop)
        !xxxxxxxxxxxxxx
        write(32, powderflow_prop)
        powderflow1 % c = c
        powderflow1 % vp = vp
        powderflow1 % A = A
        powderflow1 % B = B
        ...
        powderflow1 % h = h
        !
    end subroutine powderflow_property
    ...
    !
    !************************************************************
    subroutine initial(cells, faces, nodes, bnodes)
        !************************************************************
        implicit none
        type(vector) :: qv
        type(cell), pointer :: cells(:)
        ...
        if (s_nstart > 0) then
            ...
            if (s_ilevelset > 0) then ! level set, simple initial condition
            do i = 1, size(cells)
                cells(i) % qv % v(ils) = cells(i) % centp(2) ! half space
            end do
            end if
            ...
            if (s_nstart > 0) then
                do j = 1, s_idt
                    do i = 1, size(cells)
                        ...
                        if (laser1 % cold_start > 0.5) then
                            cells(i) % qvn(j) % v(ien) = 300.
                            cells(i) % qvn(j) % v(imb:ime) = zero
                        end if
                    end do
                end do
            end if
        ...
        if (id == 0) then
            ...
            open(7, file = 'gems.res.dat') !.'//i2s(id+1))
            if (s_nstart > 0) then
                do i = 1, s_nstart
                    read(7, *, end = 20)
                end do
                20 backspace(7) !ifile)
                s_nstart = s_nstart + 1
    ...
    subroutine read_binary(cells, faces, id, n)
        ...
        print *, 'wenda notes: we read from read_binary in dfd_input.f90'
        open(5, file = 'gems.bin.' // i2s(id), form = 'unformatted')
        read(5, end = 10) k
        if (s_nstart >= 1) s_nstart = k
        read(5, end = 10) s_elapsed_time

        print *, 'wenda notes: readin s_elapsed_time in input.f90', s_elapsed_time
        if (laser1 % cold_start > 0.5) then
            print *, 'cold_start'
        else
            print *, 'hot start'
        ...
        do i = 1, size(cells)
            !    read(5,end=10)(cells(i)%qv%v(:n),i=1,size(cells))
            read(5, end = 10) cells(i) % qv % v(:n)

            !print *,id,i,cells(i)%qv%v(ien)
            if (laser1 % cold_start > 0.5) then
                cells(i) % qv % v(ien) = 300.
                cells(i) % qv % v(imb:ime) = zero
                !print *,'new',i,cells(i)%qv%v(ien)
            end if
    ...
    subroutine output_option
        ...
        integer :: reaction_rate, levelset
        integer :: ibuff(55)
        ...
        dBxdt, dBydt, dBzdt, dExdt, dEydt, dEzdt, reaction_rate, levelset
        !
        integer :: id, ierr, i
        call mpi_comm_rank(mpi_comm_world, id, ierr)
        vname(:55) = (/&
    ...
    subroutine woutput_option(faces)
        ...
        integer :: stagnation_temperature
        integer :: ibuff(17)
        ...
        stagnation_pressure, stagnation_temperature
        !
    ...
    subroutine setup_extension(cells, faces, id, np)
        implicit none
        type(cell), pointer :: cells(:), cc
        type(face), pointer :: faces(:)
        integer :: id, np

        real(rfp) :: maxx = 0, maxy = 0, maxz = 0, minx = 100, miny = 100, minz = 100, minxall, minyall, minzall
        real(rfp) :: ptx, pty, ptz
        integer :: xvalue(500) = 1e6, yvalue(500) = 1e6, zvalue(500) = 1e6
        integer :: xvaluegather(10000) = 1e6, yvaluegather(10000) = 1e6, zvaluegather(10000) = 1e6
        integer :: xint, yint, zint
        integer :: xct = 0, yct = 0, zct = 0, xctsum, yctsum, zctsum, xctmax, yctmax, zctmax
        integer :: i, j, k, flag, ierr, temp, ma, mb, mc

        do i = 1, size(cells)
            ptx = cells(i) % centp(1)
            pty = cells(i) % centp(2)
            ptz = cells(i) % centp(3)
            xint = (ptx + 0.5e - 7)/1e - 7
            if (xint < 0) xint = (ptx - 0.5e - 7)/1e - 7
            yint = (pty + 0.5e - 7)/1e - 7
            if (yint < 0) yint = (pty - 0.5e - 7)/1e - 7
            zint = (ptz + 0.5e - 7)/1e - 7
            if (zint < 0) zint = (ptz - 0.5e - 7)/1e - 7

            !print *,id,ptx,pty,ptz
            flag = 0
            if (xct > 0) then
                do j = 1, xct
                    if (xvalue(j) == xint) then
                        flag = 1
                        exit
                    end if
                end do
        ...
        call mpi_allreduce(xct, xctsum, 1, MPI_INTEGER, MPI_SUM, mpi_comm_world, ierr)
        call mpi_allreduce(xct, xctmax, 1, MPI_INTEGER, MPI_MAX, mpi_comm_world, ierr)
        call mpi_allgather(xvalue, xctmax, MPI_INTEGER, xvaluegather, xctmax, MPI_INTEGER, mpi_comm_world, ierr)
        call rearragement(xvaluegather, xctmax * np)
        do i = 1, xctmax * np
            isize_ext = i
            if (xvaluegather(i + 1) >= 1e6 - 10) exit
        end do
        !    print *,'hahax',id,isize_ext

        call mpi_allreduce(yct, yctsum, 1, MPI_INTEGER, MPI_SUM, mpi_comm_world, ierr)
        call mpi_allreduce(yct, yctmax, 1, MPI_INTEGER, MPI_MAX, mpi_comm_world, ierr)
        call mpi_allgather(yvalue, yctmax, MPI_INTEGER, yvaluegather, yctmax, MPI_INTEGER, mpi_comm_world, ierr)
        !if (id==3) print *,id,yct,yctsum,yctmax
        !if (id==3) print *,'gather',yvaluegather(:yctmax*np)      
        call rearragement(yvaluegather, yctmax * np)
        do i = 1, yctmax * np
            jsize_ext = i
            if (yvaluegather(i + 1) >= 1e6 - 10) exit
        end do
        !    print *,'hahay',id,jsize_ext
        !if (id==3) print *,'after',yvaluegather(:yctmax*np)

        call mpi_allreduce(zct, zctsum, 1, MPI_INTEGER, MPI_SUM, mpi_comm_world, ierr)
        call mpi_allreduce(zct, zctmax, 1, MPI_INTEGER, MPI_MAX, mpi_comm_world, ierr)
        call mpi_allgather(zvalue, zctmax, MPI_INTEGER, zvaluegather, zctmax, MPI_INTEGER, mpi_comm_world, ierr)
        !if (id==0) print *,id,zct,zctsum,zctmax
        !if (id==0) print *,'gather',zvaluegather(:zctmax*np)      
        call rearragement(zvaluegather, zctmax * np)
        do i = 1, zctmax * np
            ksize_ext = i
            if (zvaluegather(i + 1) >= 1e6 - 10) exit
        end do

        isize_ext = isize_ext + 2
        jsize_ext = jsize_ext + 2
        ksize_ext = ksize_ext + 2

        allocate(xc_ext(isize_ext), yc_ext(jsize_ext), zc_ext(ksize_ext))
        xc_ext(2:isize_ext - 1) = xvaluegather(:isize_ext - 2) * 1e - 7

        dxc_ext = 1e6

        do i = 3, isize_ext - 1
            if (xc_ext(i) - xc_ext(i - 1) < dxc_ext) dxc_ext = xc_ext(i) - xc_ext(i - 1)
        end do

        xc_ext(1) = xc_ext(2) - dxc_ext
        xc_ext(isize_ext) = xc_ext(isize_ext - 1) + dxc_ext

        if (id == 0) print *, 'dxc', dxc_ext
        !if (id==0) print *,'here',xc_Ext(:)

        dyc_ext = dxc_ext
        dzc_ext = dxc_ext

        yc_ext(2:jsize_ext - 1) = yvaluegather(:jsize_ext - 2) * 1e - 7
        yc_ext(1) = yc_ext(2) - dyc_ext
        yc_ext(jsize_ext) = yc_ext(jsize_ext - 1) + dyc_ext

        zc_ext(2:ksize_ext - 1) = zvaluegather(:ksize_ext - 2) * 1e - 7
        zc_ext(1) = zc_ext(2) - dzc_ext
        zc_ext(ksize_ext) = zc_ext(ksize_ext - 1) + dzc_ext

        if (id == 0) print *, '=======dim for EXT model', isize_ext, jsize_ext, ksize_ext

        allocate(map_ext(isize_ext, jsize_ext, ksize_ext))
        ...
        do i = 1, size(cells)
            ...
            do j = 1, isize_ext
                if (xint == xvaluegather(j)) then
                    xint = j + 1
                    exit
                end if
            end do
            do j = 1, jsize_ext
                if (yint == yvaluegather(j)) then
                    yint = j + 1
                    exit
                end if
            end do
            do j = 1, ksize_ext
                if (zint == zvaluegather(j)) then
                    zint = j + 1
                    exit
                end if
            end do
            map_ext(xint, yint, zint) = i

        end do

        allocate(var1_ext(isize_ext, jsize_ext, ksize_ext))
        allocate(var2_ext(isize_ext, jsize_ext, ksize_ext))
        allocate(var3_ext(isize_ext, jsize_ext, ksize_ext))
        allocate(var4_ext(isize_ext, jsize_ext, ksize_ext))

    end subroutine setup_extension

    subroutine rearragement(input, length)
        implicit none
        integer :: input(:)
        integer :: length, i, j
        integer :: flag, temp
        ...
        do i = 2, length
            do j = 1, i - 1
                if (input(i) == input(j) .and. input(i) <= 1e6 - 10) then
        ...
        !bubble sort to increasing order
        flag = 1
        do while (flag == 1)
            flag = 0
            do i = 1, length
                if (input(i) > input(i + 1)) then
                    !print *,'change',i,input(i),input(i+1)
                    temp = input(i)
                    input(i) = input(i + 1)
                    input(i + 1) = temp
                    flag = 1
    ...
    end subroutine rearragement


end MODULE dfd_input
