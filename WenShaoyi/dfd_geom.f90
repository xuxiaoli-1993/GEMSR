...
!
  do i = 1, size(faces)
   ...
   f2n => cf%f2n
   if(ndim == 2) then
   vecn(:2) = area2d(nodes(f2n(1))%xyz,nodes(f2n(2))%xyz)
   cc => cf%left_cell
...
!   cc%vol = cc%vol + abs(vol)
   if(cf%itype ==0) then
    ...
    if( vol < zero .and.s_ifmt /= 1) vecn = -vecn
!
    cf%area = sqrt(sum(vecn*vecn))
...
!   if(s_iaxis > 0) cf%vl = cf%vl * one_third *abs(sum(nodes(f2n(:))%xyz(ia)) + cl%centp(ia))
   vecn(:2) = area2d(nodes(f2n(2))%xyz,cl%centp)
   vecn = sign(one,vol) * vecn
!   if(s_iaxis > 0) vecn = vecn * half *abs(nodes(f2n(2))%xyz(ia) + cl%centp(ia))
   cf%avec(:,1) = cf%avec(:,1) + vecn(:2)
   cf%avec(:,4) = cf%avec(:,4) + vecn(:2)
!
   vecn(:2) = area2d(cl%centp,nodes(f2n(1))%xyz)
   vecn = sign(one,vol) * vecn
!   if(s_iaxis > 0) vecn = vecn * half *abs(nodes(f2n(1))%xyz(ia) + cl%centp(ia))
   cf%avec(:,1) = cf%avec(:,1) + vecn(:2)
   cf%avec(:,3) = cf%avec(:,3) + vecn(:2)
...
!   if(s_iaxis > 0) cf%vr = cf%vr * one_third *abs(sum(nodes(f2n(:))%xyz(ia)) + cr%centp(ia))
   vecn(:2) = area2d(nodes(f2n(2))%xyz,cr%centp)
   vecn = sign(one,vol) * vecn
!   if(s_iaxis > 0) vecn = vecn * half *abs(nodes(f2n(2))%xyz(ia) + cr%centp(ia))
   cf%avec(:,2) = cf%avec(:,2) + vecn(:2)
   cf%avec(:,4) = cf%avec(:,4) + vecn(:2)
!
   vecn(:2) = area2d(cr%centp,nodes(f2n(1))%xyz)
   vecn = sign(one,vol) * vecn
