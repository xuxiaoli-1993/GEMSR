...
contains
   ...
   subroutine ludecomp(ul, ips)
      ...
      do k = 1, n - 1
         ...
         if (idxpiv /= k) then
            iswap = ips(k)
            if(idxpiv > neq .or. idxpiv < 1) then
               print*, 'error in ludecomp'
               print *, ul; stop
            end if
            ips(k) = ips(idxpiv)
            ips(idxpiv) = iswap
   ...
   function snrm2_xl(x)
      ...
      snrm2_xl = zero
      do i=1, size(x)
         snrm2_xl = snrm2_xl + x(i)**2
      end do
      snrm2_xl = sqrt(snrm2_xl)
   end function snrm2_xl
   ...
   function check_parallel(a, b) result(yon)
      implicit none
      real(rfp) :: a(ndim), b(ndim)
      real(rfp) :: a1(3), b1(3), cross(3)
      logical :: yon
      ...
      if(ndim == 1) then
         ...
         b1 = (/b(1), b(2), zero/)
         cross = acrossb(a1, b1)
         if(sqrt(dot_product(cross,cross)) <= 1e-6) then
            yon = .true.
         else
            yon = .false.
         end if
