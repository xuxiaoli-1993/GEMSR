...
contains
   ...
   subroutine output_axisy_tecplot(fn,cells,nodes,faces,interf,it)
      ...
      n = size(nodes)
      m = size(cells)
      write(2,*)'zone t="',s_elapsed_time,'",n=',n*na,',e=',m*(na-1),',f=feblock,et=brick'
      !
      ! coordinates
         ...
         do k = 1,na
            ...
            case(6)  ! vy
               ...
               if(s_iaux /= 0) then
                  ...
                  write(2,20)nodes(:)%qv%v(imb+1)*cos(theta)
               end if
            case(7)  ! vz
               theta = real(k-1,rfp) * alpha / real(na-1,rfp)
               if(s_iaux /= 0) then
                  ...
                  write(2,20)nodes(:)%qv%v(imb+1)*sin(theta)
               end if
            case(8)
               write(2,20)nodes(:)%qv%v(ien)
            case(9)
