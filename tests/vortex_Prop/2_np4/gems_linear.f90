...
contains
   ...
   contains
      ...
      subroutine point_gauss_seidel
         type(cell), pointer :: current_cell,ghost_cell
         type(face), pointer :: current_face, cf
         type(matrix), pointer ::dm
         type(vector) :: deltv
         type(vector), pointer :: res,dqv
         integer :: i,k,j

         ! check qv
         do i = 1, size(cells)
            current_cell => cells(i)
            call check_cell_inf(current_cell, i)
         end do
         ...
         do k = 1, s_isub
            do i = 1, size(cells)
               ...
               if (abs(dqv % v(1))>1e20) then
                  print *, 'dqv exploded at id', i
                  print *, 'coordinate: ', current_cell%centp
                  print *, dqv % v(:)
                  stop
               end if
      ...
      subroutine check_cell_inf(current_cell, id)
         implicit none
         type(cell), pointer :: current_cell
         integer :: i, j, id

         do j = 1, neq
            !if(abs(current_cell%qv%v(j)) > 1e20 .or. isnan(current_cell%qv%v(j))) then
            if(abs(current_cell%qv%v(j)) > 1e20 .or. ieee_is_nan(current_cell%qv%v(j))) then
               print *, 'residual exploded at id', id
               print *, current_cell%centp
               print *, current_cell%qv%v(:)
               stop
            end if
         end do
         ...
         do i = 1, neq
            do j = 1, neq
               !if(isnan(current_cell%dm%e(i,j))) then
               if(ieee_is_nan(current_cell%dm%e(i,j))) then
                  print *, 'nan at id ', id
                  print *, current_cell%centp
                  print *, current_cell%dm%e
                  stop
               end if
            end do
         end do
      end subroutine check_cell_inf

   end subroutine linear_solve
end module gems_linear
