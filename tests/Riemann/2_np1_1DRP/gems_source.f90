...
contains
   ...
   subroutine source(cells, nodes)
      ...
      if (g_igrav == 1 .or. metal%Gravity_on == 1) call gravity_source(cells)
   ...
   subroutine solid_melting(cells)
      ...
      type(vector), pointer :: res, qv
      type(vector) :: qvf
      real(rfp) :: gl, kc, volume, darcy, w(ndim), un, h, tau, hl, hs
      real(rfp) :: darcyv(ndim)
      integer :: i, it, j
      !
      ...
      do i = 1, size(cells)
         ...
         if(metal%Darcy_on == 1) then
            ! Darcian damping force
            ! Kozeny-Carman equation

            gl = liquid_fraction(qv) + 1.e-20_rfp

            if((one-gl)**2 < 1e-20) then
               kc = myhuge
            else
               kc = metal % k0 * gl**3 / (one - gl)**2
            end if
            !
            darcy = -mumix(qv) / kc * volume
            res % v(imb:ime) = res % v(imb:ime) + darcy * (qv % v(imb:ime)-darcyv)
            w = -darcy
            call diagadd(cc % dm % e(imb:ime, imb:ime), w) ! source Jacobian
         end if

         if(metal%Thermal_Buoyancy_on == 1) then
            ! Buoyancy force
         ...
         if(metal%Enthalpy_Correction_on == 1) then
            !  correct enthalpy
            ! 
            do j = 1, size(cc % sface)
               cf => cc % sface(j) % to_face
               qvf = (cf % left_cell % qv + cf % right_cell % qv) * half
               un = rhof(qvf) * sum(cf % vecn * (qvf % v(imb:ime)-darcyv)) * cf % area
               if (associated(cc, cf % right_cell)) un = -un
               gl = liquid_fraction(qvf)
               hl = metal%cpl * (qvf%v(ien)-metal%tl) + metal%lheat_melting + metal%cps * metal%ts
               hs = metal%cps * qvf%v(ien)
               res % v(ien) = res % v(ien) - (un * (one - gl) * (hl - hs))
   ...
   subroutine gravity_source(cells)
      ...
      do i = 1, size(cells)
         ...
         !source jacobian
         dj1 = dj(qv)
         sqv = source_vector(current_cell)

         dm = dm - dj1 * volume ! source jacobian: subtraction
         current_cell % res = current_cell % res + sqv * volume !source residual
      end do
   ...
   subroutine vis_add(cc, div, dvdr, nodes)
      ...
      div = div / cc % vol
      if(naux > 0) dvdr = dvdr / cc % vol - cc % qv % v(iaub) / cc % centp(s_iaxis)
