module gems_data
   ...
   integer :: s_nsave !--- frequency to invoke save_binary
   ...
   real(kind = rfp) :: s_dt, s_dt_inv !--- inversion of real time step(deltT)
   real(kind = rfp) :: s_elapsed_time
   real(kind = rfp) :: s_dx  ! characteristic mesh size
   ...
   !
   real(kind = rfp) :: g_runi = 8314.9 !--- universal gas constant [J/K kmol]
   real(kind = rfp) :: g_patm = 1.013e5_rfp !--- 1atm = 101.3Kpa
   real(kind = rfp) :: g_troom = 278.0 ! Kelvin
   !
   real(kind = rfp) :: g_mwi(nspe) !--- molecular weight [kg/kmol] or [g/mol]
   real(kind = rfp) :: g_zi(nspe) !--- compressibility factor of ideal gas law
   real(kind = rfp) :: g_rho0(nspe) !--- some reference density
   real(kind = rfp) :: g_r !--- gas constant
   real(kind = rfp) :: g_gm !--- the ratio of specific heats
   real(kind = rfp) :: g_cpi(nspe) !--- the specific heat at constant pressure [J/kg K]
   ...
   ! debug
   type(cell), pointer :: db_moncell => NULL()
   integer :: db_cellid, db_dummy, db_pretime, db_pid
   integer :: db_nadv
   real(rfp) :: db_xc, db_yc, db_zc

end module gems_data
