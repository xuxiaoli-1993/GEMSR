...
contains
   ...
   subroutine solution_information
      ...
      if (s_ivis == 2.and.id == 0) then
         ...
         if (nteq /= 2) then
            print *, 'please set nteq = 2 in als_constant.f90'
            print *, 'Re-compile before you run the case'
            call mpi_abort(mpi_comm_world, ierr, ierr)
         end if
      end if
      ...
      if (s_nstart > 0) then
         ...
         case(91)
            ...
            do i=1,size(cells)
               ...
               ! for molten metal
               cells(i)%qv%v(ien) = 4000.
            end do
         case(99)
            ! custom
            ymax=10_rfp - 0.005_rfp
            do i = 1, size(cells)
               y=cells(i)%centp(2)
               p_gravity = rhof(cells(i)%qv) * (ymax-y) * maxval(abs(g_gravity))
               cells(i)%qv%v(ico) = cells(i)%qv%v(ico) + p_gravity
            end do
         end select
      end if
      !
