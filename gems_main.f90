...
program gems_main
   ...
   do nadv = s_nstart, s_nend
      ...
      if(id == 0 .and. mod(nadv,s_nt)==0) print 401,'nadv=',nadv,'p_dt=',1/s_dt_inv,'etime=',s_elapsed_time
...
contains
   ...
   subroutine check_conv(cells, faces, nadv)
      ...
      real(rfp) :: rate, l2q, vm, vm_max, pcfl, pcfl_all
      real(rfp), allocatable :: sumqt(:), errt(:), errk(:), sumq(:), errk2(:), errt2(:)
      integer :: nadv, ifile, i, neqt, k, id, neqx, sz
      ...
      neqt = neqx * k
      allocate(sumqt(neqt), errt(neqt), displs(k), rcount(k), errt2(neqt))
      ...
      vm_max = zero
      do i=1, size(cells)
         vm = norm2(cells(i)%qv%v(imb:ime))
         if(vm_max < vm) vm_max = vm
      end do

      pcfl = vm_max / 0.01 / s_dt_inv
      ...
      call mpi_allreduce(MPI_IN_PLACE, pcfl, 1, MPI_DOUBLE, MPI_MAX, mpi_comm_world, ierr)
      ...
      deallocate(sumqt, errt, displs, rcount, errt2)
