...
!    if(j < 10) then
!     write(fn,'("gems_mon_",i1,".plt")')j
!    else if(j < 100) then
!     write(fn,'("gems_mon_",i2,".plt")')j
!    else if(j < 1000) then
!     write(fn,'("gems_mon_",i3,".plt")')j
!    else if(j < 10000) then
!     write(fn,'("gems_mon_",i4,".plt")')j
!    end if
print *,id,'printing files'

    fn = "./output/gems_mon_"//trim(i2s(j))//".plt"
    !call output_to_tecplot(fn,cells,nodes,faces,interf,id)
    call output_cell_tecplot(fn,cells,nodes,faces,id) !use cell-based output by Xuxiao
    fn = "./output/gemsb."//trim(i2s(j))//".label"
    call wall_plot(fn,cells,nodes,faces,interf,id)
 end subroutine output_result

  subroutine output_to_plot(cells,nodes,faces,interf,it)
implicit none
!
  type(cell),pointer::cells(:)
  type(node),pointer::nodes(:)
...
! head   
  if(ndim == 2) then
  write(2,*)'zone t="',int(s_elapsed_time*s_dt_inv+0.5),'",n=',size(nodes),',e=',size(cells), &
            ', f=feblock,et=quadrilateral'
  else
  !try this:
  write(2,*) 'zone t="',int(s_elapsed_time*s_dt_inv+0.5),'",n=',size(nodes),',e=',size(cells),',f=feblock,et=brick'
...
! head   
  if(ndim == 2) then
    ...
    write(2,*)'zone t="',int(s_elapsed_time*s_dt_inv+0.5),'",n=',size(nodes),',e=',size(cells),',f=feblock',  &
            ',varlocation=(4=cellcentered),et=brick'
  else
    write(2,*)'zone t="',int(s_elapsed_time*s_dt_inv+0.5),'",n=',size(nodes),',e=',size(cells),',f=feblock',  &
...
10 format(10I8)
 ...
 select case(nd)
  ...
  if(id==0) print*, 'save bindary'
  open(5,file='gems.bin.'//i2s(id),form='unformatted')
  rewind(5)
  write(5)n        !,err%v !,iscp%cfl
    write(5) s_elapsed_time

!print *,'wenda output s_elapsed_time',s_elapsed_time
  do k = 1, size(cells)
!  write(5)(cells(k)%qv,k=1,size(cells))
  write(5)cells(k)%qv
  end do
...
!
  do j = 1, s_idt
   do k = 1, size(cells)
!    write(5)(cells(k)%qvn(gear_in(j)),k=1,size(cells))
    write(5)cells(k)%qvn(gear_in(j))
   end do
  end do

  if(usels>0 .and. s_idt>0) then
    do k = 1,size(cells)
      write(5) cells(k)%ls
    end do
  end if
  close(5)
!  
!  open(5,file='dfd.adap',form='unformatted')
!  open(5,file='dfd.adap')
!  rewind(5)
!  write(5)(nodes(k)%qv%v(:ien),k=1,size(nodes))
!  write(5,'(4i8)')2, 2, size(nodes),2
!  do k=1,size(nodes)
!  write(5,'(5e30.16)')nodes(k)%qv%v(ico),nodes(k)%qv%v(imb)
!  end do
!  close(5)
...
subroutine wenda_output(cells,faces,np,id,nadv) 
    implicit none
    type(face),pointer::faces(:),cf
    type(cell),pointer::cells(:),cc
    integer::np,id,nadv,index   

    integer::i,j,k,ii,jj,kk,ierr,itn,n

    double precision,allocatable::var0_temp(:,:,:),var1_temp(:,:,:),var2_temp(:,:,:)
    double precision,allocatable::var3_temp(:,:,:),var4_temp(:,:,:),var5_temp(:,:,:)
    double precision,allocatable::var6_temp(:,:,:),var7_temp(:,:,:)
    real(rfp)::out(20)
...
!     if(s_imonit == 0) return
!     n = s_imonit*s_nt
!     if( mod(nadv,n) /= 0) return
!     itn = nadv / n
    itn=nadv

    call mpi_barrier(mpi_comm_world, ierr)
    allocate(var0_temp(isize_ext,jsize_ext,ksize_ext))
    allocate(var1_temp(isize_ext,jsize_ext,ksize_ext))
    allocate(var2_temp(isize_ext,jsize_ext,ksize_ext)) 
    allocate(var3_temp(isize_ext,jsize_ext,ksize_ext))
    allocate(var4_temp(isize_ext,jsize_ext,ksize_ext))
    allocate(var5_temp(isize_ext,jsize_ext,ksize_ext))
    allocate(var6_temp(isize_ext,jsize_ext,ksize_ext))
    allocate(var7_temp(isize_ext,jsize_ext,ksize_ext))
        ...
        do j=1,jsize_ext
            do k=1,ksize_ext
                if (map_ext(i,j,k)>0) then
                    var1_temp(i,j,k) = cells(map_ext(i,j,k))%ls
                    var2_temp(i,j,k) = cells(map_ext(i,j,k))%qv%v(ien)
                    var3_temp(i,j,k) = cells(map_ext(i,j,k))%qv%v(imb)
                    var4_temp(i,j,k) = cells(map_ext(i,j,k))%qv%v(imb+1)
                    var5_temp(i,j,k) = cells(map_ext(i,j,k))%qv%v(imb+2)
                    var6_temp(i,j,k) = cells(map_ext(i,j,k))%ls
                    var7_temp(i,j,k) = cells(map_ext(i,j,k))%ls
                end if            
            end do
        end do     
    end do

    call mpi_allreduce(var1_temp,var0_temp,isize_ext*jsize_ext*ksize_ext,MPI_DOUBLE_PRECISION,MPI_SUM,mpi_comm_world,ierr)
    call mpi_allreduce(var2_temp,var1_temp,isize_ext*jsize_ext*ksize_ext,MPI_DOUBLE_PRECISION,MPI_SUM,mpi_comm_world,ierr) 
    call mpi_allreduce(var3_temp,var2_temp,isize_ext*jsize_ext*ksize_ext,MPI_DOUBLE_PRECISION,MPI_SUM,mpi_comm_world,ierr)
    call mpi_allreduce(var4_temp,var3_temp,isize_ext*jsize_ext*ksize_ext,MPI_DOUBLE_PRECISION,MPI_SUM,mpi_comm_world,ierr)
    call mpi_allreduce(var5_temp,var4_temp,isize_ext*jsize_ext*ksize_ext,MPI_DOUBLE_PRECISION,MPI_SUM,mpi_comm_world,ierr)
    call mpi_allreduce(var6_temp,var5_temp,isize_ext*jsize_ext*ksize_ext,MPI_DOUBLE_PRECISION,MPI_SUM,mpi_comm_world,ierr)
    call mpi_allreduce(var7_temp,var6_temp,isize_ext*jsize_ext*ksize_ext,MPI_DOUBLE_PRECISION,MPI_SUM,mpi_comm_world,ierr)
    ...
    do i=1,isize_ext
        do j=1,jsize_ext
            do k=1,ksize_ext
                ...
                var0_temp(i,j,k) = var0_temp(ii,jj,kk)
                var1_temp(i,j,k) = var1_temp(ii,jj,kk)
                var2_temp(i,j,k) = var2_temp(ii,jj,kk) 
                var3_temp(i,j,k) = var3_temp(ii,jj,kk)
                var4_temp(i,j,k) = var4_temp(ii,jj,kk)
                var5_temp(i,j,k) = var5_temp(ii,jj,kk)
                var6_temp(i,j,k) = var6_temp(ii,jj,kk)
            end do
        end do     
    end do
...
!6   format (1x,20e20.6)
    ...
    if (id==0) then           
        print *,'printing wenda_output',nadv

        fn = "./output/wenda_output_"//trim(i2s(itn))//".dat"     
        open(43,file=fn)
        write(43,*) 'TITLE="vector"'
        !write(43,*) 'VARIABLES="x(m)" "y(m)" "z(m)" "levelset" "T" "Vx" "Vy" "Vz" "VAR1" "VAR2"'
	write(43,*) 'VARIABLES="x(m)" "y(m)" "z(m)" "levelset"'
        !write(43,*) 'ZONE T="',int(s_elapsed_time*s_dt_inv+0.5), '" I=',ISIZE_ext-2,' J=',JSIZE_ext-2,' K=',KSIZE_ext-2,' F=POINT'
	write(43,*) 'ZONE T="',nadv, '" I=',ISIZE_ext-2,' J=',JSIZE_ext-2,' K=',KSIZE_ext-2,' F=POINT'
        DO k=2,KSIZE_ext-1
        DO j=2,JSIZE_ext-1
        DO i=2,ISIZE_ext-1  
            ...
            out(2) = yc_ext(j)
            out(3) = zc_ext(k)
            out(4) = var0_temp(i,j,k)
            out(5) = var1_temp(i,j,k)
            out(6) = var2_temp(i,j,k)
            out(7) = var3_temp(i,j,k)
            out(8) = var4_temp(i,j,k)
            out(9) = var5_temp(i,j,k)
            out(10) = var6_temp(i,j,k)
    ...
    deallocate(var0_temp,var1_temp,var2_temp,var3_temp,var4_temp,var5_temp,var6_temp,var7_temp)
    call mpi_barrier(mpi_comm_world, ierr)

end subroutine wenda_output


end MODULE dfd_output
