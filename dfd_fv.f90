...
! 	    end if
    ...
    subroutine facefluxscheme(cf, cl, cr, qvl, qvr, flux, aa, tl, tr, ajl, ajr)
        ...
        case(4) ! Rusanov's (TVD Lax_Friedrich)
            ...
            flux = evl * tl + evr * tr - aa * (qvr - qvl)
        case(5) ! AUSM+
            !      call eigenvalues(s_ipre,tl,sl,vecn,qvl,rhoppav)
            !      call eigenvalues(s_ipre,sr,tr,vecn,qvr,rhoppav)
            !      sl = min(sl,c2)
            !      sr = max(sr,c1)
            !       tr = (min(sr,zero) - min(zero,sl)) / (sr - sl) * a
            !       tl = (a - tr )
    ...
    subroutine lhs(cells, faces, nodes)
        ...
        do i = 1, size(cells)
            ...
            if (s_idt > 0) then ! unsteady
                gear(:ndt1) = current_cell % vol * gear_c
                gm = gamma(0, qv, rhopp, cells(i), one, s_cspeed) ! without precondition
                res => current_cell % res
                !
                res = res - conservative_qv(qv) * gear(1)
                do j = 1, s_idt
                    res = res - conservative_qv(current_cell % qvn(gear_in(j))) * gear(j + 1)
                end do
                dm = dm + gm * gear(1)
                !  
            end if
    ...
    subroutine viscous(faces, nodes, cells)
        ...
        real(rfp) :: dl, dr, vel(ndim), un, p, rho, volume_inv, epsi, alpha
        integer, pointer :: f2n(:)
        ...
        if (s_ivis == 0 .and. s_irec /= 2) return ! Euler eq
...
! 	    end if
            ...
            qvf = sum_qv_node(nodes(f2n(:))) * coef
        ...
        !print *,cells(1070)%res%v(ien),'1070'
        !print *,cells(7336)%res%v(ien),'7336'
    ...
    function diff_coef(qv, cf)result(c)
        ...
        if (nspe > 1) zd = mass_diffusivity(qv)
        ! turbulent model
        if (s_ivis >= 2) then
            if (s_ivis == 2) then !K-Omega
                zmut = mu_turb(qv)
