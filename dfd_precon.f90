...
contains
    ...
    subroutine precondition_cell(switch, cells, nodes, nadv) !cc,rhopp)
        ...
        type(cell), pointer :: cc, cl, cr, cells(:)
        type(face), pointer :: cf
        integer, intent(in) :: switch
        type(vector) :: qv, qvc
        !  real(rfp),intent(out)::rhopp
        real(rfp) :: u2, v2, dl, mu, um, c2, coef, rhop, ug
        integer :: i, k, nadv, nmax = 100
        ...
        do k = 1, size(cells)
            cc => cells(k)
            qvc = cc % qv
            um = max(dot_product(qvc % v(imb:ime), qvc % v(imb:ime)), 1.e-5_rfp) !cf%vecn) 
            if (switch == 4) goto 10
            do i = 1, size(cc % sface)
                cf => cc % sface(i) % to_face
                cl => cf % left_cell
                cr => cf % right_cell
                qv % v = (cl % qv % v + cr % qv % v) * half
                u2 = dot_product(qv % v(imb:ime), qv % v(imb:ime)) !cf%vecn) 
                um = max(um, u2)
                !     if(g_igrav > 0) then  ! gravity preconditioning
                !       u2 = sum(g_gravity * (cr%centp-cl%centp))**2 / max(u2,1.e-10_rfp)
                !       um = max(um,u2)
                !     end if
                !
                if (switch == 5) then
                ...
                if (s_idt > 0) then ! unstedy preconditioning
                    v2 = dl * s_dt_inv
                    v2 = v2 * v2
                    um = max(um, v2)
                end if
            ...
            rhop = rhopf(qvc)
            if (switch == 2) um = max(um, abs(qv % v(ico))/rhof(qv) * quarter)
            if (switch == 6) um = max(um, s_vnn) ! cut-off
            !    if(switch == 3) then
            10 continue
            !    print *,um,rhop
            !     if(nadv < nmax) &
